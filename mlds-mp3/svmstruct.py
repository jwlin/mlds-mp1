...
def init_model(sample, sm, sparm):
    ...
    sm.num_features = 39#max(max(x) for x,y in sample)[0]+1
    sm.num_classes = 48#max(y for x,y in sample)
    sm.size_psi = sm.num_features*sm.num_classes +48*48
...
def read_examples(filename, sparm):
    ...
    for i in xrange(len(TestParser.x_seq)):
        sparse_vec.append((i, TestParser.x_seq[i]))
    print len(sparse_vec)
    print  len(sparse_vec[0]),len(TestParser.y_hat[0])
    

    examples = []
    assert len(sparse_vec) == len(TestParser.y_hat)
    for i in xrange(len(sparse_vec)):
        #assert len(sparse_vec[i]) == len(TestParser.y_hat[i])
        tup = (sparse_vec[i], TestParser.y_hat[i])
        examples.append(tup)'''
...
def loss(y, ybar,sparm):
    ...
    print  ''.join(y_c)
    print ''.join(ybar_c)
    return Levenshtein.distance(''.join(y_c), ''.join(ybar_c))
